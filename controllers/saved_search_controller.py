...
from services.saved_search_service import create_search_record, delete_search_historyAll, delete_single_search_history,get_user_search_history
...
async def get_history(current_user: dict = Depends(get_current_user)):
    ...
    """
    histories = await get_user_search_history(user_id=current_user["user_id"])
    if histories:
        return histories
...
async def delete_history_by_search_id(search_id: str, current_user: dict = Depends(get_current_user)):
    ...
    """
    deleted_count = await delete_single_search_history(search_id=search_id)
    if deleted_count:
...
# Delete by User ID
@router.delete("/delete-all-history")
async def delete_history_by_user_id(current_user: dict = Depends(get_current_user)):
    ...
    """
    deleted_count = await delete_search_historyAll(user_id=current_user["user_id"])
    if deleted_count:
